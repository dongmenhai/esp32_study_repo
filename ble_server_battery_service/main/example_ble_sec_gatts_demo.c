...
#define EXAMPLE_DEVICE_NAME                       "ESP_BLE_BATTERY"
...
static uint16_t battery_handle_table[BAS_IDX_NB];
...
// config adv data
static esp_ble_adv_data_t battery_adv_config = {
    .set_scan_rsp = false,
...
static esp_ble_adv_data_t battery_scan_rsp_config = {
    .set_scan_rsp = true,
    .include_name = true,
...
static esp_ble_adv_params_t battery_adv_params = {
    .adv_int_min        = 0x100,
    .adv_int_max        = 0x100,
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SET_LOCAL_PRIVACY_COMPLETE_EVT:
        ...
        esp_err_t ret = esp_ble_gap_config_adv_data(&battery_adv_config);
        if (ret){
        ...
        ret = esp_ble_gap_config_adv_data(&battery_scan_rsp_config);
        if (ret){
            ESP_LOGE(GATTS_TABLE_TAG, "config adv data failed, error code = %x", ret);
        }else{
            adv_config_done |= SCAN_RSP_CONFIG_FLAG;
...
void app_main(void)
{
    ...
    xTaskCreate(battery_notify_task, "battery_notify_task", 4096, NULL, 10, &battery_handle);
